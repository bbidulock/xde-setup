...
if [ ! -f "$priv/rc" -o "$priv/rc.m4" -nt "$priv/rc" ]; then
	export VTWM_TYPE="$prog"
	export VTWM_VERSION="$vers"
	export VTWM_CONFIG_HOME="$priv"
	export VTWM_CONFIG_SDIR="$sdir"
	export VTWM_RCFILE="$priv/rc"
	export VTWM_M4FILE="$priv/rc.m4"

	runscript "getstyles" "$prog" "$vers" "$priv/rc"
	runscript "config"    "$prog" "$vers" "$priv/rc"
fi

makelink "$HOME" ".vtwmrc"    "$priv/rc"
makelink "$HOME" ".vtwmrc.m4" "$priv/rc.m4"
