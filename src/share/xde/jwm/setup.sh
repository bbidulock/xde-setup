...
name=jwm
files="version config defaults desktop getstyles groups keys menu paths rc root setstyle style tray"
xtras=""
clone="version config getstyles setstyle"
...
if [ ! -f "$priv/rc" -o "$priv/rc.m4" -nt "$priv/rc" ]; then
	export JWM_TYPE="$prog"
	export JWM_VERSION="$vers"
	export JWM_CONFIG_HOME="$priv"
	export JWM_CONFIG_SDIR="$sdir"
	export JWM_RCFILE="$priv/rc"

	runscript "getstyles" "$prog" "$vers" "$priv/rc"
	runscript "config"    "$prog" "$vers" "$priv/rc"
fi

makelink "$HOME" ".${name}rc"    "$priv/rc"
